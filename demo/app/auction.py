...
from app.transformer_bot import transformer_decision
from app.game_settings import BB, SB, show_game
    ...
    while number_decisions != number_player and not every_fold:
        ...
        for player in player_list:
            ...
            if not player.decision and player.live:
                ...
                human_player = next(player for player in Player.player_list_chair if player.kind == "human")
                bot_player = next(
                    player for player in Player.player_list_chair if player.name == HandHistoryTracker.bot_name
                )
                pot_before_action = max(float(pot_table), 1.0)
                bot_stack_before = int(bot_player.stack)
                human_stack_before = int(human_player.stack)
                ...
                if player.kind == 'human':
                ...
                elif player.kind == 'transformer':
                    decision = transformer_decision(
                        bot_player=player,
                    ...
                    from app.utils import custom_argmax
                    ...
                    from app.utils import get_content
                ...
                # Now we have decision from one of player: human, bot, ai, gpt, transformer
                ...
                wager_amount = int(chips if decision in {"raise", "call", "all-in"} else 0)
                HandHistoryTracker.record_decision(
                    player_name=player.name,
                    decision=decision,
                    chips=wager_amount,
                    pot_before=pot_before_action,
                    bot_stack=bot_stack_before,
                    human_stack=human_stack_before,

...
def poker_round(player_id):
    ...
    bot_player = next(player for player in player_list_chair if player.kind != "human")
    human_player = next(player for player in player_list_chair if player.kind == "human")
    HandHistoryTracker.start_hand(
        bot_name=bot_player.name,
        human_name=human_player.name,
        sb_name=player_list[-2].name,
        bb_name=player_list[-1].name,
        stacks={player.name: player.stack for player in player_list_chair},
    )
    ...
    if player_list[-1].stack > BB:
        ...
        HandHistoryTracker.record_blind(player_list[-1].name, float(BB), pot_before=1.0)
    else:
        ...
        HandHistoryTracker.record_blind(
            player_list[-1].name,
            float(player_list[-1].input_stack),
            pot_before=1.0,
        )
    ...
    if player_list[-2].stack > SB:
        ...
        HandHistoryTracker.record_blind(
            player_list[-2].name,
            float(SB),
            pot_before=max(float(player_list[-1].input_stack), 1.0),
        )
    else:
        ...
        HandHistoryTracker.record_blind(
            player_list[-2].name,
            float(player_list[-2].input_stack),
            pot_before=max(float(player_list[-1].input_stack), 1.0),
        )
    ...
    if number_live_players + number_allin_players == 1:
        ...
        common_cards = flop
        HandHistoryTracker.set_street(common_cards)
